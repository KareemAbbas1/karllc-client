...
import { getCountry, getState } from "../logistics-page-components/getCountry";

// eslint-disable-next-line react/prop-types
const CountryList = ({ setFormData }) => {
    ...
    useEffect(() => {
        const cont = getCountry();
        getState();
        const list = document.getElementById("country_list");
        const defaultCountrycode = Array.from(list.getElementsByTagName("option")).filter(el => el.dataset.countrycode === cont)
        setPhoneState((prev) => {
            return {
                ...prev,
                countrycode: defaultCountrycode[0].value,
                countryImg: `https://flagpedia.net/data/flags/w580/${defaultCountrycode[0].dataset.countrycode.toLocaleLowerCase()}.webp`
            }
        })
    ...
    const selectImgHandler = () => {
        ...
        setPhoneState((prev) => {
            return { ...prev, countryImg: `https://flagpedia.net/data/flags/w580/${select.selectedOptions[0].dataset.countrycode.toLocaleLowerCase()}.webp` }
        })
    }
    ...
    return (
        ...
        >
            <img className="w-7 h-5 rounded" src={phoneState.countryImg} alt=""
            />
            ...
            <select
                ...
                id="country_list"
                className="w-[70%] bg-dark rounded-lg border-none cursor-pointer"
